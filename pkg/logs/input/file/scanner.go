...
func (s *Scanner) cleanup() {
	...
	for _, tailer := range s.tailers {
		stopper.Add(tailer)
		delete(s.tailers, tailer.GetScanKey())
	}
	stopper.Stop()
}
...
func (s *Scanner) startNewTailer(file *File, m config.TailingMode) bool {
	...
	log.Infof("Starting a new tailer for: %s (offset: %d, whence: %d) for scanner key %s", file.Path, offset, whence, file.GetScanKey())
...
func (s *Scanner) restartTailerAfterFileRotation(tailer *Tailer, file *File) bool {
	log.Info("Log rotation happened to", tailer.File.GetScanKey())
